...
// Tampilkan/sembunyikan baris filter & pencarian (generic).
function toggleFilterRow(rowId, arrowId){
  var row = document.getElementById(rowId);
  if(!row) return;
  var open = row.style.display === 'block';
  row.style.display = open ? 'none' : 'block';
  var arrow = document.getElementById(arrowId);
  if(arrow) arrow.classList.toggle('open', !open);
}

// Toggle cari & filter sesi di bawah "Riwayat Sesi".
function toggleSesiFilter(which){
  var up = which === 'pc' ? 'Pc' : 'M';
  toggleFilterRow('sesiFilterRow'+up, 'sesiFilterArrow'+up);
}
