...
function renderDashboard(){
  ...
  try {
    ...
    if(bl) bl.textContent = 'Update real-time • '+(BULAN[parseInt(curMonth,10)]||'')+' '+curYear;
    ...
    ['lux-chart-note','lux-chart-note-pc'].forEach(function(id){ var e=document.getElementById(id); if(e) e.innerHTML='<span class="msym" style="font-size:14px;vertical-align:-2px;color:#047857">info</span> '+noteTxt; });
    // Tren vs bulan lalu
...
// Quick actions hero luxury mobile
function luxCatatHadir(){
  try { goMob('absen'); } catch(e){}
  try { if(typeof showMobAbsenMain==='function') setTimeout(showMobAbsenMain, 220); } catch(e){}
}
function luxKasMasuk(){
  try { goMob('kas'); } catch(e){}
  try { if(typeof openKasTambah==='function') setTimeout(openKasTambah, 220); } catch(e){}
}
  ...
  if(hasData){
    kasLux =
      '<div class="lux-kas">'+
        ...
        '<span>Kondisi kas: Sehat &amp; Terverifikasi</span></div>'+
        '<div class="lux-kas-ic" style="background:#d1fae5;color:#0f5132"><span class="msym" style="font-size:24px">account_balance</span></div></div>'+
        '<div class="lux-kas-row lux-kas-in"><div class="lux-kas-ic" style="background:rgba(209,250,229,.7);color:#047857;width:36px;height:36px"><span class="msym" style="font-size:22px">arrow_downward</span></div>'+
        '<div><small>Pemasukan Bulan Ini</small><strong>+'+fmtRp(masukBulan)+'</strong><span>Dari iuran generus &amp; donatur</span></div></div>'+
        '<div class="lux-kas-row lux-kas-out"><div class="lux-kas-ic" style="background:rgba(254,226,226,.7);color:#dc2626;width:36px;height:36px"><span class="msym" style="font-size:22px">arrow_upward</span></div>'+
        '<div><small>Pengeluaran Bulan Ini</small><strong>-'+fmtRp(keluarBulan)+'</strong><span>Konsumsi &amp; perlengkapan sesi</span></div></div>'+
      '</div>';
  }
...
function buildDashChart(sesiStats){
  ...
  var SERIES = [
    {vals:A_arr, color:'#dc2626', grad:'dashGradA', op:'0.18', name:'Alpa'},
    {vals:H_arr, color:'#198754', grad:'dashGradH', op:'0.22', name:'Hadir'},
    {vals:I_arr, color:'#d97706', grad:'dashGradI', op:'0.18', name:'Izin'}
  ];
  ...
  for(var val=0; val<=maxVal; val+=step){
    var yy = ty(val);
    var isBase = (val===0);
    grid += '<line x1="'+pL+'" y1="'+yy.toFixed(1)+'" x2="'+(W-pR)+'" y2="'+yy.toFixed(1)+'" stroke="'+(isBase?'#cfc9bd':'#e9e5db')+'" '+(isBase?'stroke-width="1.2"':'stroke-dasharray="3 3" stroke-width="1"')+'/>';
  ...
  var xlbls = sesiStats.map(function(s,i){
    ...
    return '<line x1="'+x+'" y1="'+pT+'" x2="'+x+'" y2="'+baseY+'" stroke="#f0ede6" stroke-width="1"/>'+
      '<text x="'+x+'" y="'+(H_-8)+'" text-anchor="middle" font-size="11" font-weight="600" fill="#6b7280" font-family="\'Plus Jakarta Sans\',sans-serif">'+
